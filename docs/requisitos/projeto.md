...
| RF-03 | Motor de consolidação e geração de PDF | O sistema processa as respostas de todos os alunos e gera os relatórios em PDF por professor, estruturados por turma, contendo as médias dos 6 critérios de avaliação, a pontuação de cada turma e a pontuação consolidada final, exatamente conforme o modelo padrão do campus (RN-09). | Must | D-01 |
...
| RN-09 | Relatório Consolidado de Avaliação | O relatório em PDF por docente deve seguir o modelo padrão (Cabeçalho MEC/IFTO, dados do Docente/Matrícula) com tabelas agrupadas por turma contendo a média dos 6 critérios estáticos (Q1 a Q6), a linha "PONTUAÇÃO DA TURMA (SOMATÓRIA DAS MÉDIAS DIVIDIDO POR DOIS)" e a linha final "MÉDIA DA PONTUAÇÃO DAS TURMAS CONSOLIDADO- RESULTADO FINAL" (média simples das pontuações das turmas), além de localidade, data e área de assinatura. Os 6 critérios de avaliação são estáticos e descritos nas diretrizes de geração do PDF. | UC-03, UC-05 |
