...
3. O sistema calcula a média de cada um dos 6 critérios de avaliação para cada turma vinculada ao professor. A pontuação de cada turma é a somatória das médias dividida por dois. O Resultado Final (Média da Pontuação das Turmas Consolidado) é a média simples das pontuações das turmas avaliadas.
4. O sistema gera um PDF institucional por professor contendo cabeçalho do IFTO, metadados do docente, tabelas separadas por turma com os 6 critérios e médias, pontuação de cada turma, o Resultado Final consolidado, local, data e área para assinatura digital, conforme RN-09.
